---
layout: page
title: Global MHD Simulation of Earth's Magnetosphere
description: Coupled MHD + RCM simulation of the magnetosphere
img: assets/img/area_mhd_simulation.jpg
importance: 1
category: research
---

Earth's magnetosphere is the giant region of space carved out by the solar wind and the planet's magnetic field. Global magnetohydrodynamic (MHD) simulation, coupled with kinetic models of the inner magnetosphere, is the primary tool we have for studying how plasma circulates and accelerates on system-wide scales. The main model I use is [**MAGE**](https://cgs.jhuapl.edu/MAGE/) (Multiscale Atmosphere–Geospace Environment), an actively developed coupled MHD + RCM + ionospheric framework that I help develop.

Within this area I work on the following projects:

- **O$^+$ ion transport from the cusp to the inner magnetosphere.** A study of how O+ ions originating in the cusp are transported to the inner magnetosphere.
- **BBF contribution to the ring current.** A backward test-particle tracer (BackTrac) used to identify which fraction of storm-time ring-current ions originate from Bursty Bulk Flows (BBFs) in the plasma sheet.
